...
import { exportTotalConversion, importTotalConversion } from '../data/totalConversion';
import { useModding } from '../store/moddingStore';

export const GameSyncPanel: React.FC = () => {
  const { refreshMods, refreshBase, getAllCardsWithSource, baseCards } = useModding();
  ...
  const exportRegistries = () => {
    // export COMPLETO: registros + total conversion
    const full = { ...exportAllRegistries(), totalConversion: exportTotalConversion() };
    const blob = new Blob([JSON.stringify(full, null, 2)], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'moddingbuild_full.cargasmod.json'; a.click();
  ...
  const importRegistries = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      importAllRegistries(data);
      if (data.totalConversion) importTotalConversion(data.totalConversion);
      refreshMods(); alert('✅ Mod completo importado');
  ...
  return (
    <div className="space-y-3">
      <div className="bg-gradient-to-r from-emerald-900/40 to-slate-900 rounded-xl p-4 border border-emerald-800/50">
        ...
        <p className="text-xs text-slate-400">ModdingBuild detecta el juego en la misma ruta de instalación y comparte cartas, personajes, mods, tags, efectos y mecánicas.</p>
      </div>
      ...
      <div className="bg-slate-900 rounded-xl p-4 border border-slate-800">
        <h4 className="text-xs font-black text-slate-300 mb-2">📦 Exportar MOD COMPLETO (tags, efectos, habilidades, pasivas, mecánicas, reglas, modos, estados, fases, recursos, victoria, tableros, IA, keywords, UI)</h4>
      ...
      <div className="text-[0.6rem] text-slate-500 bg-slate-800/40 rounded-lg p-3 border border-slate-700/40 leading-relaxed">
        ℹ️ <b>Cómo funciona la sinergia:</b> al instalar el juego y ModdingBuild en la misma carpeta, ambos usan <code className="text-cyan-300">AppLocalData/com.cargas.game</code>.
        ModdingBuild escribe en <code className="text-cyan-300">mods/</code>, <code className="text-cyan-300">dlc/</code> y los registros; el juego los lee al arrancar (<code className="text-cyan-300">loadModsFromInstallFolder()</code>).
      </div>
    </div>
  );
};

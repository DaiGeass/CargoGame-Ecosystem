...
export const ContentBrowser: React.FC = () => {
  ...
  const exportAll = () => {
    ...
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `moddingbuild_export.json`; a.click();
    URL.revokeObjectURL(url);
  };
  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      <div className="max-w-4xl mx-auto space-y-6">
        {/* intro */}
        <div className="bg-gradient-to-br from-rose-950/50 to-slate-900 rounded-2xl p-5 border border-rose-800/50">
          <h1 className="text-xl font-black text-rose-300">🧪 ModdingBuild — Herramienta de modding de CARGAS</h1>
          <p className="text-sm text-slate-400 mt-1">Recolecta el contenido <b>directamente del juego</b> y permite modding <b>agresivo</b>: cartas, personajes, habilidades, pasivas, efectos, sinergias, <b>mecánicas nuevas</b> y reglas globales. Por GUI o CLI. Reconocida por CARGAS en la misma ruta de instalación.</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={() => { setFilterSource('all'); setView('cards'); }} className="bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-bold px-4 py-2 rounded-lg">🎨 Editor de Cartas</button>
          <button onClick={() => setView('tc')} className="bg-rose-600 hover:bg-rose-500 text-white text-sm font-bold px-4 py-2 rounded-lg">🌐 Total Conversion</button>
          <button onClick={() => setView('synergy')} className="bg-fuchsia-600 hover:bg-fuchsia-500 text-white text-sm font-bold px-4 py-2 rounded-lg">⚡ Sinergias</button>
        ...
        <div className="bg-slate-900 rounded-2xl p-4 border border-slate-800">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => { setFilterSource('base'); setView('cards'); }} className="text-xs font-bold bg-slate-800 hover:bg-slate-700 text-white px-3 py-1.5 rounded-lg">Ver cartas →</button>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { if (newModName.trim()) { createMod(newModName.trim(), 'ModdingBuild'); setNewModName(''); } }}
          ...
          <div className="space-y-2">
            {mods.map((mod) => (
              <div key={mod.manifest.id} className="bg-slate-900 rounded-xl p-3 border border-slate-800 flex items-center gap-3">
                ...
                <button onClick={() => { setFilterSource(mid(mod)); setView('cards'); }} className="text-xs bg-slate-800 hover:bg-slate-700 text-white px-2.5 py-1 rounded-lg">Ver</button>

...
import { totalConversionStats } from '../data/totalConversion';
import { getCustomMechanics, getCustomEffects, getCustomTags } from '../data/registries';
...
export const SystemDiagnosticsPanel: React.FC = () => {
  ...
  const run = async () => {
    ...
    results.push({ label: 'Sinergia con juego', status: 'ok', detail: 'ModdingBuild escribe en cargas.installedMods.v1 = mismo storage que CARGAS ✓' });

    const tc = totalConversionStats();
    results.push({ label: 'Total Conversion', status: 'ok', detail: `${tc.gameModes} modos · ${tc.statusEffects} estados · ${tc.resources} recursos · ${tc.boards} tableros · ${tc.aiProfiles} IAs · ${tc.keywords} keywords` });
    results.push({ label: 'Modding agresivo', status: 'ok', detail: `${getCustomMechanics().length} mecánicas · ${getCustomEffects().length} efectos custom · ${getCustomTags().length} tags custom` });

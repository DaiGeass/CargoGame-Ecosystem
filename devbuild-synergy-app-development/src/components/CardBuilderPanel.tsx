...
export const CardBuilderPanel: React.FC = () => {
  const { startEditCard, setView } = useDevBuild();
  const [name, setName] = useState('Mi Carta Custom');
  const [type, setType] = useState<CardType>('damage');
  const [description, setDescription] = useState('Una carta creada con DevBuild');
  ...
  const exportAsMod = () => {
    const card = buildCard();
    const mod = { manifest: { name: `Mod: ${name}`, author: 'DevBuild', version: '1.0.0', description: `Carta: ${name}` }, cards: [card] };

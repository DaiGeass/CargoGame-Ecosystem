// ============================================================
// DEVBUILD STORE - Estado central (usa el sistema REAL de mods)
// ============================================================
...
export type DevBuildView = 'browser' | 'visual' | 'synergy' | 'code' | 'preview' | 'bridge';
...
const blankCard: PlayableCard = {
  id: '', name: 'Nueva Carta', type: 'damage', value: -30,
...
export const useDevBuild = create<DevBuildState>((set, get) => {
  return {
    ...
    saveEditingCard: (targetSourceId) => {
      ...
      // limpiar metadata de SourcedCard si la hubiera
      delete (card as any).__source; delete (card as any).__sourceId; delete (card as any).__sourceName;

      if (targetSourceId === 'base') {
        // Editar carta base => persistir como OVERRIDE del juego
        setCardOverride(card.id, card);
        set((s) => {
    ...
    deleteCard: (cardId, sourceId) => {
      if (sourceId === 'base') {
        deleteBaseCard(cardId); // persiste la eliminación como override
    ...
    emitBridge: (type, payload) => {
      const log = { id: uid('msg'), type, from: 'devtool', to: 'broadcast', payload, ts: Date.now() };
...
setTimeout(() => {
  useDevBuild.getState().refreshBase();
}, 800);
